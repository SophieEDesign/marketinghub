    ...
    <div className="flex flex-wrap gap-2 mb-4">
      {filters.map((filter) => {
        ...
        return (
          <div
            key={filter.id}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-100 dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-700"
          >
            <span className="font-medium">{fieldLabel}</span>
            <span className="text-gray-500 dark:text-gray-400">{operatorLabel}</span>
            {valueLabel && <span className="font-medium">{valueLabel}</span>}
            <button
              onClick={() => onRemoveFilter(filter.id)}
              className="ml-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full p-0.5 transition"
              title="Remove filter"
            >
              <X className="w-3 h-3" />
            </button>
