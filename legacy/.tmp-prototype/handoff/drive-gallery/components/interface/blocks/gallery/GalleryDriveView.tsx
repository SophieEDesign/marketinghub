// components/interface/blocks/gallery/GalleryDriveView.tsx
"use client"
...
export default function GalleryDriveView({ rootFolderId, title, subtitle, className }: Props) {
  ...
  // Reset search when navigating between folders.
  ...
  const header = useMemo(() => {
    ...
    return { title: title ?? "Shared Image Gallery", subtitle: subtitle ?? "Approved marine photography by vessel type and sector.", webViewLink: null as string | null }
    ...
    >
        ...
        <div className="flex min-w-0 items-start gap-3.5">
          {inFolder && (
            <button
              ...
              className="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[10px] border border-[#d4d7dc] bg-white hover:bg-[#f7f9fb]"
            >
              ...
              .map((f) => {
                ...
                return (
                  <button
                    ...
                    className="group block overflow-hidden rounded-[15px] border border-[#e4e7ec] bg-white text-left shadow-[0_1px_2px_rgba(31,42,68,0.04)] transition-all hover:-translate-y-[3px] hover:border-[#d7dce3] hover:shadow-[0_16px_34px_rgba(31,42,68,0.16)]"
                  >
            ...
            {state.data.images
              ...
              .map((im) => (
                <button
                  ...
                  className="group relative aspect-square overflow-hidden rounded-[13px] border border-[#e4e7ec] bg-[#eef1f4] hover:shadow-[0_12px_26px_rgba(31,42,68,0.16)]"
                >
      ...
      {lightbox && (
        <div
          ...
          className="fixed inset-0 z-[60] flex items-center justify-center bg-[rgba(9,17,27,0.82)] p-10"
        >
