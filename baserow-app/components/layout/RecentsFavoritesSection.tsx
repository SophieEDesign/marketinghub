...
export default function RecentsFavoritesSection({ primaryColor: _primaryColor }: RecentsFavoritesSectionProps) {
  ...
  return (
    <>
      {favorites.length > 0 && (
        <div className="py-2 border-t border-hub-border">
          <div className="px-3 mb-1">
            <button
              onClick={() => toggleSection("favorites")}
              className="w-full flex items-center justify-between px-2 py-1.5 text-xs font-medium uppercase tracking-wider text-muted-foreground hover:bg-muted/60 rounded-lg transition-colors"
            >
              <span>Starred</span>
              ...
              {favorites.slice(0, 10).map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(sidebarNavItemClassName(isActive), "w-full")}
                  >
